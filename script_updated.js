...
(function () {
    ...
    function runTracking() {
      ...
      // ---- UTM params (read from URL, persist in cookie for cross-page attribution)
      ['utm_source', 'utm_medium', 'utm_campaign', 'utm_content', 'utm_term', 'utm_audience'].forEach(function (p) {
        var val = getParam(p) || getCookie('lp_' + p);
        if (getParam(p)) setCookie('lp_' + p, getParam(p), 30);
        setFieldValue(p, val);
      });

      // ---- Google Ads IDs (passed as ValueTrack params in final URL)
      setFieldValue('campaign_id',   getParam('campaign_id'));
      setFieldValue('ad_group_id',   getParam('ad_group_id'));
      setFieldValue('field_beb550d', getParam('ad_id'));
    }
