...
def activity_dashboard():
    ...
    # ── Agent Team Roster ─────────────────────────────────────────────────────
    # Real 9-agent org: 1 manager + 3 departments (Performance, CRO/LP, Support)
    # "roles" maps to agent_activity_log.role values for action counts.
    _TEAM_DEFS = [
        # ── Orchestrator ────────────────────────────────────────────────────
        {
            "title":         "AI Orchestrator",
            "desc":          "Manager over all 3 departments. Routes work, gates every write action (✅), owns cross-dept handoffs.",
            "color":         "#7c3aed",
            "dept":          None,
            "dept_color":    None,
            "is_lead":       False,
            ...
            "charter":       [
                ...
                "Gates all write actions (pause / scale / create) with ✅/❌",
                "Owns the 8-step daily intelligence loop",
        ...
        # ── Performance dept ────────────────────────────────────────────────
        {
            ...
            "desc":          "Sets KPI thresholds, channel mix, budget allocation. Gates ✅/❌ on every paid action.",
            "color":         "#dc2626",
            "dept":          "Performance",
            "dept_color":    "#dc2626",
            "is_lead":       True,
            ...
            "charter":       [
                ...
                "Allocates budget across Meta / Google / Snap / LinkedIn",
                "Triages paid-media flags to Campaign Manager or Creative Strategist",
                "Final approval on every pause / scale decision",
            ],
            "receives_from": ["AI Orchestrator"],
            "sends_to":      ["Campaign Manager", "Creative Strategist"],
            "agent_file":    ".claude/agents/performance-lead.md",
        },
        {
            "title":         "Campaign Manager",
            "desc":          "12-field naming spec, both Meta pixels, keyword-policy buckets, audience setup.",
            "color":         "#f97316",
            "dept":          "Performance",
            "dept_color":    "#dc2626",
            ...
            "charter":       [
                ...
                "Attaches Qoyod_CRM_PIXEL + Qoyod_Web_PIXEL on every Meta campaign",
                "Manages keyword-policy buckets (always-negative, brand-only, competitor)",
                "Configures audiences: Interests, Lookalike, Retargeting, Broad",
            ],
            "receives_from": ["Performance Lead"],
            "sends_to":      [],
            "agent_file":    ".claude/agents/campaign-manager.md",
        },
        {
            "title":         "Creative Strategist",
            "desc":          "OCEAN persona mapping, A/B creative variants per segment, LP asset alignment with CRO.",
            "color":         "#f59e0b",
            "dept":          "Performance",
            "dept_color":    "#dc2626",
            ...
            "charter":       [
                ...
                "Scopes A/B creative variants per audience x channel",
                "Aligns landing-page assets with CRO Specialist before tests go live",
                "Runs in parallel with Campaign Manager under Performance Lead",
            ],
            "receives_from": ["Performance Lead"],
            "sends_to":      ["CRO Specialist"],
            "agent_file":    ".claude/agents/creative-strategist.md",
        },
        # ── CRO / LP dept ───────────────────────────────────────────────────
        {
            "title":         "CRO Specialist",
            "desc":          "8-section LP brief, test hypothesis, success criteria from 14-day CPQL data. Chain lead.",
            "color":         "#059669",
            "dept":          "CRO / LP",
            "dept_color":    "#059669",
            "is_lead":       True,
            ...
            "sprite":        "cro",
            "charter":       [
                "Writes 8-section LP brief + test hypothesis for every experiment",
                "Defines success criteria from 14-day CPQL + destination_url data",
                "Owns test-result decisions — escalates to Performance Lead if KPIs change",
                "First link in the CRO > UI/UX > Developer handoff chain",
            ],
            "receives_from": ["AI Orchestrator", "Creative Strategist"],
            "sends_to":      ["UI/UX Designer"],
            "agent_file":    ".claude/agents/cro-specialist.md",
        },
        {
            "title":         "UI/UX Designer",
            "desc":          "LP variant designs aligned to OCEAN personas. ZATCA badge above the fold. Hands to Developer.",
            "color":         "#0d9488",
            "dept":          "CRO / LP",
            "dept_color":    "#059669",
            "roles":         {"lp_design", "ui_ux"},
            "sprite":        "designer",
            "charter":       [
                "Turns CRO brief into annotated LP design (Figma-ready)",
                "Ensures ZATCA compliance badge is above the fold on every design",
                "Aligns visual language to OCEAN persona for the target segment",
                "Includes interaction notes so Developer has zero ambiguity",
            ],
            "receives_from": ["CRO Specialist"],
            "sends_to":      ["Developer"],
            "agent_file":    ".claude/agents/ui-ux-designer.md",
        },
        {
            "title":         "Developer",
            "desc":          "Builds LP variants, wires UTM passthrough, fires both Meta pixels, deploys + verifies.",
            "color":         "#0891b2",
            "dept":          "CRO / LP",
            "dept_color":    "#059669",
            "roles":         {"lp_deploy"},
            "sprite":        "developer",
            "charter":       [
                "Implements LP variant from UI/UX design spec",
                "Wires utm_source / utm_medium / utm_campaign / utm_content on every form field",
                "Fires Qoyod_CRM_PIXEL + Qoyod_Web_PIXEL — verifies in Events Manager before sign-off",
                "Deploys to production — last link in the CRO chain",
            ],
            "receives_from": ["UI/UX Designer"],
            "sends_to":      [],
            "agent_file":    ".claude/agents/developer.md",
        },
        # ── Support dept ────────────────────────────────────────────────────
        {
            "title":         "Marketing Ops",
            ...
            "charter":       [
                ...
                "Monitors both Meta pixels (Qoyod_CRM + Qoyod_Web) health",
                "Owns HubSpot lead_utm_campaign field mapping",
                "Rotates Railway env-vars and credentials — no hardcoded secrets",
            ],
            "receives_from": ["AI Orchestrator"],
            "sends_to":      [],
        ...
        },
        {
            ...
            "desc":          "Owns memory/. 8-step loop on live BQ, period comparisons, forecasts, CRO A/B results.",
            "color":         "#8b5cf6",
            "dept":          "Support",
            "dept_color":    "#6366f1",
            ...
            "charter":       [
                ...
                "Runs the 8-step intelligence loop on live BQ — never cached data",
                "Period comparisons (current vs prior window) via analysers/period_compare.py",
                "Monthly forecasts: spend / leads / SQLs / ROAS via analysers/forecaster.py",
            ],
            "receives_from": ["AI Orchestrator"],
            "sends_to":      [],
            "agent_file":    ".claude/agents/growth-analyst.md",
        },
    ]
    ...
    for _m in _TEAM_DEFS:
        ...
        # Recent actions: last 5 distinct action strings (most-recent-day first)
        _seen_acts: set = set()
        _recent: list = []
        for _a, _cnt, _d in sorted(_matched, key=lambda x: x[2] or "", reverse=True):
            _label = _a.replace("_", " ").title()
            if _label not in _seen_acts:
                _seen_acts.add(_label)
                _recent.append({"action": _label, "day": str(_d) if _d else None, "count": _cnt})
            if len(_recent) >= 5:
                break
        team_roster.append({
            "title":         _m["title"],
            ...
            "color":         _m["color"],
            "dept":          _m.get("dept"),
            "dept_color":    _m.get("dept_color"),
            "roles":         sorted(_mroles),
            "sprite":        _m.get("sprite", "default"),
            "is_main":       _m.get("is_main", False),
            "is_lead":       _m.get("is_lead", False),
            "actions_30d":   _c30,
            "actions_7d":    _c7,
            "last_day":      str(_last_day) if _last_day else None,
            "last_action":   (_last_act or "").replace("_", " ").title() if _last_act else "—",
            "charter":       _m.get("charter", []),
            "receives_from": _m.get("receives_from", []),
            "sends_to":      _m.get("sends_to", []),
            "agent_file":    _m.get("agent_file", ""),
