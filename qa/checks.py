...
#   - new_biz   = Sales Pipeline + Bookkeeping + Qflavours (3 pipelines)
#   - all       = every pipeline (Renewals, QoyodK, Partnerships, etc.)
# Both must reconcile within thresholds for the gate to pass.
# Added 2026-05-20 (incident); expanded 2026-05-20 to cover both scopes.
NEW_BIZ_PIPELINES = ("Sales Pipeline", "Bookkeeping", "Qflavours")


def _reconcile_deals_scope(scope_name: str, pipeline_labels: tuple[str, ...] | None,
                           start, end, since_ms: int, until_ms: int) -> dict | None:
    """Run BQ↔HS reconcile for one scope. pipeline_labels=None means 'all pipelines'."""
    import requests
    c, p, d = _bq()
    ...
    if pipeline_labels is None:
        ...
        plabels = ",".join(f"'{x}'" for x in pipeline_labels)
        where = f"date BETWEEN '{start}' AND '{end}' AND pipeline IN ({plabels})"
    sql = f"""
    ...
    # Resolve pipeline labels → IDs (skip if scope = all)
    pipeline_ids = None
    if pipeline_labels is not None:
        r = requests.get("https://api.hubapi.com/crm/v3/pipelines/deals",
                         headers={"Authorization": f"Bearer {token}"}, timeout=15)
        all_p = r.json().get("results", [])
        label_to_id = {pp["label"]: pp["id"] for pp in all_p}
        pipeline_ids = [label_to_id[l] for l in pipeline_labels if l in label_to_id]
        if not pipeline_ids:
            return None
    ...
    while True:
        ...
        if pipeline_ids:
            # HubSpot Search API uses IN with values array
            filters.append({
...
def check_deals_full_reconcile(
    ...
    """Pass if both reportable scopes reconcile:
      1. new_biz  = Sales Pipeline + Bookkeeping + Qflavours
      2. all      = every pipeline
    Each verified on counts (total + won) and amounts (USD: total + won).
    Drift > threshold on EITHER scope → block.
    Window: last 7 settled days (T-2..T-8 Riyadh).
    """
    import datetime as _dt
    def _q():
        riyadh = _dt.timezone(_dt.timedelta(hours=3))
        ...
        until_ms = int(_dt.datetime((end + _dt.timedelta(days=1)).year,
                                    ...
                                    tzinfo=riyadh).timestamp() * 1000)
        return {
            "new_biz": _reconcile_deals_scope("new_biz", NEW_BIZ_PIPELINES,
                                              start, end, since_ms, until_ms),
            "all":     _reconcile_deals_scope("all", None,
                                              start, end, since_ms, until_ms),
        }

    try:
        results = _cached("deals_recon", _q)
        if not results or not results.get("new_biz") or not results.get("all"):
            return QACheckResult(name="deals_full_reconcile", passed=True, severity="warn",
                                 detail="could not load scope data — non-fatal")
        ...
        scope_summaries = []
        all_failures = []
        for scope_name, m in results.items():
            ...
            if abs(d_total) > count_threshold:
                failures.append(f"[{scope_name}] deals_total drift {d_total:+.2%}")
            if abs(d_won)   > count_threshold:
                failures.append(f"[{scope_name}] deals_won drift {d_won:+.2%}")
            if abs(a_total) > amount_threshold:
                failures.append(f"[{scope_name}] amount_total drift {a_total:+.2%}")
            if abs(a_won)   > amount_threshold:
                failures.append(f"[{scope_name}] amount_won drift {a_won:+.2%}")
            all_failures.extend(failures)
            scope_summaries.append(
                f"{scope_name}: BQ deals={m['bq_deals_total']}/{m['bq_deals_won']} "
                f"amt=${m['bq_amount_total']:.0f}/${m['bq_amount_won']:.0f}"
            )

        ok = not all_failures
        detail = " | ".join(scope_summaries)
        if all_failures:
            detail = "; ".join(all_failures) + " || " + detail
        return QACheckResult(
